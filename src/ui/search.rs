...
pub fn render(f: &mut Frame, app: &App, area: Rect) {
    ...
    let result_block = Block::default()
        ...
        .borders(Borders::ALL)
        .border_style(if !app.search_mode_input { Style::default().fg(Color::Yellow) } else { Style::default() });
    ...
    let items: Vec<ListItem> = app.search_results.iter().enumerate().map(|(i, r)| {
        let res = string::truncate(app.display.display_node(&r.resource), col_w);
        ...
        let line = Line::from(vec![
            Span::styled(format!("{:<width$}", res, width = col_w), Style::default().fg(Color::Yellow)),
            Span::raw(" │ "),
            Span::styled(format!("{:<width$}", prop, width = col_w), Style::default().fg(Color::Cyan)),
            Span::raw(" │ "),
    ...
    if !app.search_mode_input { state.select(Some(app.search_selection)); }
