...
pub fn render(f: &mut Frame, app: &App, area: Rect) {
    ...
    let result_block = Block::default()
        .title(vars_title)
        .borders(Borders::ALL)
        .border_style(if !app.sparql_mode_input { Style::default().fg(Color::Yellow) } else { Style::default() });

    let inner = result_block.inner(chunks[1]);
    f.render_widget(result_block, chunks[1]);

    let items: Vec<ListItem> = app.sparql_result.as_ref().map(|r| {
        let ncols = r.variables.len().max(1);
        let col_w = (inner.width as usize).saturating_sub(ncols.saturating_sub(1) * 3) / ncols;        
        r.rows.iter().enumerate().map(|(i, row)| {
            let spans: Vec<Span> = row.iter().enumerate().flat_map(|(j, cell)| {
                let val = cell.as_ref().map(|t| app.display.display_term_plain(t))
                    .unwrap_or_default();
                let color = super::util::cell_color(cell);
                let padded = format!("{:<width$}", val, width = col_w);
                if j + 1 < ncols { 
                    vec![Span::styled(padded, Style::default().fg(color)), Span::raw(" │ ")] 
                } else { 
    ...
    if !app.sparql_mode_input { state.select(Some(app.sparql_selection)); }
