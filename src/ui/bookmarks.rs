...
pub fn render(f: &mut Frame, app: &App, area: Rect) {
    ...
    let items: Vec<ListItem> = app.bookmarks.iter().enumerate().map(|(i, uri)| {
        let label = app.display.display_node(uri);
        let show_raw = label == format!("<{}>", uri.as_str());
        let line = if show_raw {
            Line::from(Span::styled(format!("  {}", uri.as_str()), Style::default().fg(Color::Yellow)))
        } else {
            Line::from(vec![
                Span::styled(format!("  {} ", label), Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
                Span::styled(format!("<{}>", uri.as_str()), Style::default().fg(Color::DarkGray)),
            ])
        };
    ...
    state.select(if app.bookmarks.is_empty() { None } else { Some(app.bookmarks_selection) });
